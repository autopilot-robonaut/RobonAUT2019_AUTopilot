...
uint8_t direction=0,konwncross=0; //utolso keresztezodes azert 2 elemu mert az is fontos h honna jovunk
...
uint8_t num_of_unkonown_road=0;
uint8_t next_dir;
...
uint8_t data_process(uint8_t direction,int32_t x,int32_t y,uint32_t distance,int8_t LANE_CHANGE){
	...
	if(newcross){ 
		// ha meg nem volt meg ilyen keresztezodes akkor hozzáfuzzük 
	...
	if(konwncross<9){				
		...
		data_table[current_cross][direction+2]=last_back_decision;
		if(direction==0){ // ha normál irányból jövünk és van döntési leheoségünk
			...
			decision_list[0][1]=next_direction(direction);
		}
		else{
		...
		if(LANE_CHANGE!=-1){
			if(LANE_CHANGE==2){ 
				if(direction!=0) data_table[current_cross][11]=direction;
				else data_table[current_cross][11]=99;
			}
			else{ 
				if(last_cross[0]!=-1){
					if(last_back_decision!=last_cross[0])data_table[last_cross[0]][11]=last_cross[1];
					else data_table[last_cross[0]][11]=99;
				}
				else{
					if(direction==0) data_table[current_cross][11]=100;
					else if(direction==1) data_table[current_cross][11]=101;
		...
		if(last_cross[0] !=-1){ // csak azért kell h a legelso ne fusson hibára
			...
			data_table[current_cross][direction+5]=last_cross[0];	
			data_table[last_cross[0]][last_cross[1]+8]=distance;
			data_table[current_cross][direction+8]=distance;	
		}	
		last_cross[0]=current_cross;
		last_cross[1]=next_direction(direction);

		ismeretlen_utak();
		if(num_of_unkonown_road==0){
			path_100();
			cross_99=path_99();
			if(cross_99 !=-1){
			...
			if(csp==current_cross && direction==0) next_dir=data_table[csp][11];
			else{
				if(direction!=0){
					next_dir=0;
				}
		...
		else next_dir=next_direction(direction);
...
uint8_t next_direction(uint8_t direction){
	uint8_t next_dir;
	if(direction!=0)next_dir=0;
...
uint8_t dijkstra(int G[MAX][MAX],int n,int startnode, int LANE_CHANGE){
	...
	for(i=0;i<n;i++){
		if((index!=startnode && (data_table[index][6]==-1 || data_table[index][7]==-1 ) && LANE_CHANGE==-1)|| LANE_CHANGE==index){			
			j=index;
			do{
				last_node=j;
				j=pred[j];
...
uint8_t ismeretlen_utak(void){
	num_of_unkonown_road=0;
	for(uint8_t i=0;i<8;i++){
		for(uint8_t j=0;j<3;j++){
			if(data_table[i][j+5]==-1)  num_of_unkonown_road++;
		}
	}
	num_of_unkonown_road=num_of_unkonown_road/2;
	return (num_of_unkonown_road);
}
...
void make_cost_table_99(uint8_t goal, uint8_t from){
	...
	for(uint8_t i=0;i<NODES_NUM;i++){
		if(i!=from && G_99[i][goal] < G_99[from][goal]){
			G_99[i][goal]=-1;
		}
	}
}
uint32_t dijkstra_99(int G[MAX][MAX],int n,int startnode, int LANE_CHANGE){	
	uint32_t cost[MAX][MAX],distance[MAX],pred[MAX];
